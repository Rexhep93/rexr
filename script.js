...
function getKijktipWeeks(tips){
  ...
  var weekMap={};
  var curWeek=getCurrentISOWeek();
  tips.forEach(function(t){
    if(!t._week)return;
    /* Only include weeks within 10 weeks back from current week */
    var weekDiff=(curWeek.year-t._week.year)*52+(curWeek.week-t._week.week);
    if(weekDiff<0||weekDiff>10)return;
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  (function(){
    ...
    var sStartY=0,sDistY=0,sActive=false;
    sheetEl.addEventListener('touchstart',function(e){
      /* Only start if at top of sheet scroll or on drag handle area */
      if(sheetEl.scrollTop>0)return;
      sStartY=e.touches[0].clientY;sDistY=0;sActive=true;
    },{passive:true});
    sheetEl.addEventListener('touchmove',function(e){
      if(!sActive)return;
      sDistY=e.touches[0].clientY-sStartY;
      if(sDistY<0){sDistY=0;return;}
      sheetEl.style.transition='none';
      sheetEl.style.transform='translateY('+sDistY+'px)';
      overlayEl.style.background='rgba(0,0,0,'+Math.max(0,0.5-sDistY/400)+')';
    },{passive:true});
    sheetEl.addEventListener('touchend',function(){
      ...
      if(sDistY>100){
        ...
        setTimeout(function(){closeModal();sheetEl.style.transform='';sheetEl.style.transition='';overlayEl.style.background='';},250);
      }else{
        sheetEl.style.transform='';
        overlayEl.style.background='';
        setTimeout(function(){sheetEl.style.transition='';},250);
      }
      sDistY=0;
    });
  })();

  /* ── Horizontal swipe to change day (only on stream tab) ── */
  (function(){
    var mainEl=document.getElementById('main');
    var hStartX=0,hStartY=0,hLocked=false,hActive=false;
    mainEl.addEventListener('touchstart',function(e){
      hStartX=e.touches[0].clientX;hStartY=e.touches[0].clientY;hLocked=false;hActive=false;
    },{passive:true});
    mainEl.addEventListener('touchmove',function(e){
      if(hLocked)return;
      var dx=e.touches[0].clientX-hStartX;
      var dy=Math.abs(e.touches[0].clientY-hStartY);
      /* If vertical movement dominates, lock out horizontal swipe */
      if(!hActive&&dy>Math.abs(dx)){hLocked=true;return;}
      if(Math.abs(dx)>20){hActive=true;}
    },{passive:true});
    mainEl.addEventListener('touchend',function(e){
      if(!hActive||hLocked||activeTab!=='stream')return;
      var dx=e.changedTouches[0].clientX-hStartX;
      if(Math.abs(dx)<50)return;
      var curIdx=allDays.indexOf(selectedDate);
      if(curIdx===-1)return;
      if(dx<-50&&curIdx<allDays.length-1){
        /* Swipe left → next day */
        selectDate(allDays[curIdx+1]);
        var a=document.querySelector('.dtab.active');if(a)a.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});
      }else if(dx>50&&curIdx>0){
        /* Swipe right → previous day */
        selectDate(allDays[curIdx-1]);
        var a2=document.querySelector('.dtab.active');if(a2)a2.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});
      }
    });
  })();
