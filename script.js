  ...
  if(item._type==='live'){
    ...
    if(hasEpInfo){
      ...
      if(completeDrop&&totalEps&&airedEps>=totalEps){
        /* Heel seizoen in één keer gedrop → alleen "Nieuw seizoen" */
        epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';
      }else if(isFirstEp){
        /* Eerste aflevering van nieuw seizoen → alleen "Nieuw seizoen" */
        epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';
      }else{
        /* Gewone aflevering → twee aparte badges */
        if(totalEps){
