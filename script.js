...
function openModal(rawId){
  ...
  chain.then(function(){
    ...
    /* If overview is English, try to fetch NL version from TMDB */
    if(rawOverview&&resolvedTmdbId&&/^[A-Za-z\s]{20,}/.test(rawOverview.slice(0,60))){
      tmdb('/'+(_type==='movie'?'movie':'tv')+'/'+resolvedTmdbId,{language:'nl-NL'}).then(function(nlData){
        if(nlData&&nlData.overview&&currentModalItem===item){sd.textContent=nlData.overview;}
      }).catch(function(){});
    }
...
document.addEventListener('DOMContentLoaded',function(){
  console.log('[StreamGids] Init v1.3.0');
