...
function getVoorJouItems(){
  ...
  if(!topGenres.length){
    /* Fallback: if no genres from saved items, use popular genre IDs (action, comedy, drama, thriller, scifi) */
...
/* Enrich saved items that are missing genre data */
    ...
    if(rawOverview&&resolvedTmdbId&&/^[A-Za-z\s]{20,}/.test(rawOverview.slice(0,60))){
      tmdb('/'+(_type==='movie'?'movie':'tv')+'/'+resolvedTmdbId,{language:'nl-NL'}).then(function(nlData){
        if(nlData&&nlData.overview&&currentModalItem===item){
          ...
          /* Show English indicator if no NL translation found */
...
function fetchSpinPool(){
  ...
  return Promise.all([
    tmdb('/trending/all/week',{language:'nl-NL',region:'NL',page:1}).catch(function(){return{results:[]};}),
    tmdb('/trending/all/week',{language:'nl-NL',region:'NL',page:2}).catch(function(){return{results:[]};}),
    tmdb('/trending/all/week',{language:'nl-NL',region:'NL',page:3}).catch(function(){return{results:[]};}),
    tmdb('/trending/movie/week',{language:'nl-NL',region:'NL',page:2}).catch(function(){return{results:[]};}),
    tmdb('/trending/tv/week',{language:'nl-NL',region:'NL',page:2}).catch(function(){return{results:[]};})
  ]).then(function(pages){
    var r=[];var seen={};
    pages.forEach(function(d){(d.results||[]).forEach(function(x){if(!seen[x.id]){seen[x.id]=true;r.push(x);}});});
    spinCache=r;return r;
  });
}

function weightedRandom(items){
  /* Weighted by popularity - more popular items have higher chance */
  var totalWeight=0;
  items.forEach(function(i){totalWeight+=(i.popularity||1);});
  var rand=Math.random()*totalWeight;var cumulative=0;
  for(var j=0;j<items.length;j++){
    cumulative+=(items[j].popularity||1);
...
function renderSpinKijk(){
  ...
  sec.innerHTML='<div class="spin-hdr"><div class="spin-title">Spin & Kijk</div><div class="spin-sub">Wat kijken we vanavond?</div></div>'+
    ...
    '<div class="spin-stage" id="spinStage">'+
      '<div class="spin-idle">'+
        '<div class="spin-wheel-wrap"><div class="spin-wheel" id="spinWheel">'+
          '<svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/><line x1="12" y1="2" x2="12" y2="5"/><line x1="12" y1="19" x2="12" y2="22"/><line x1="2" y1="12" x2="5" y2="12"/><line x1="19" y1="12" x2="22" y2="12"/><line x1="4.93" y1="4.93" x2="6.34" y2="6.34"/><line x1="17.66" y1="17.66" x2="19.07" y2="19.07"/><line x1="4.93" y1="19.07" x2="6.34" y2="17.66"/><line x1="17.66" y1="6.34" x2="19.07" y2="4.93"/></svg>'+
        '</div></div>'+
...
function doSpin(sec){
  ...
  var wheel=sec.querySelector('#spinWheel');
  ...
  fetchSpinPool().then(function(pool){
    /* Filter by type */
    var filtered=pool;
    if(spinFilter==='movie')filtered=pool.filter(function(i){return i.media_type==='movie'||(!i.name&&i.title);});
    if(spinFilter==='tv')filtered=pool.filter(function(i){return i.media_type==='tv'||(i.name&&!i.title);});

    /* Exclude already-seen items */
    var excludedIds=spinExcluded.map(function(e){return e.id;});
    filtered=filtered.filter(function(i){return excludedIds.indexOf(i.id)===-1;});
    ...
    if(!filtered.length){
      ...
      sec.querySelector('#spinResetBtn').addEventListener('click',function(){spinExcluded=[];updateSpinExInfo();renderSpinKijk();});
      return;
    }

    /* Weighted random pick */
    var pick=weightedRandom(filtered);
    var isTV=pick.media_type==='tv'||(pick.name&&!pick.title);
    var type=isTV?'tv':'movie';

    /* Check NL streaming availability */
    tmdb('/'+type+'/'+pick.id+'/watch/providers').then(function(pd){
      ...
      /* Render result with delay for animation feel */
      setTimeout(function(){
        haptic('heavy');
        renderSpinResult(sec,stage,pick,type,streamer,sColor,nlLink,isTV);
      },800);
    }).catch(function(){
      spinCurrent={pick:pick,type:type,streamer:'',sColor:'var(--green)',nlLink:null,isTV:isTV};
      setTimeout(function(){
        haptic('heavy');
        renderSpinResult(sec,stage,pick,type,'','var(--green)',null,isTV);
      },800);
    });
  });
}
...
function updateSpinExInfo(){
  ...
  if(spinExcluded.length>0){
    ...
    if(resetLink)resetLink.addEventListener('click',function(){spinExcluded=[];updateSpinExInfo();showToast('Filters gereset');});
  }else{
...
function doSearch(q){
  ...
  Promise.all([tmdb('/search/movie',{query:q,language:'nl-NL',region:'NL'}).catch(function(){return{results:[]};}),tmdb('/search/tv',{query:q,language:'nl-NL',region:'NL'}).catch(function(){return{results:[]};})]).then(function(res){
    ...
    /* Build items immediately without provider info */
    ...
    /* Lazy-load provider info in batches of 5 */
...
function renderKijktipContent(tip){
  ...
  p.then(function(d){
    ...
    /* "Waarom deze tip" tag based on genres */
    var whyTag='';
