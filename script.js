        ...
        .then(function(detail){
          ...
          if(best){
            ...
            .then(function(seasonDetail){
              ...
              /* Bewaar air_date van eerste aflevering om nieuw seizoen te detecteren */
              ...
              epDateMap[show.id].seasonFirstAirDate=firstEpDate;
            });
          }
        });
      }));
      ...
      tvShows.map(function(t){
        ...
        return Object.assign({},t,{
          ...
          _epInfo:ep?{
            ...
            seasonFirstAirDate:ep.seasonFirstAirDate||null
          }:null
        });
      })
    );
      ...
      bw=bw.then(function(){
        return Promise.all(batch.map(function(m){
          ...
          .then(function(pd){
            ...
            /* TMDB air dates zijn US-datums. In NL is content vaak
               1 dag later beschikbaar. Verschuif TV dates +1 dag. */
            if(epDate&&m._isTV){
              var d=new Date(epDate+'T12:00:00');
              d.setDate(d.getDate()+1);
              epDate=d.toISOString().slice(0,10);
            }
...
/* Voor TV: haal altijd detail op (search-resultaten bevatten geen episode info) */
if(item._type===‘tv’&&item.tmdb_id){
...
var firstEpDate=(eps.length>0&&eps[0].air_date)?eps[0].air_date:null;
/* Match de juiste aflevering bij deze GS datum.
TMDB air_dates zijn US, GS datums zijn NL (+1 dag).
Zoek de aflevering waarvan air_date+1 dag het dichtst bij item._date ligt. */
var matchedEp=best;
...
/* +1 dag voor NL beschikbaarheid */
var nlDate=new Date(ep.air_date+‘T12:00:00’);
...
html=’<button class="filter-chip back-chip" data-sf="back">←</button>’;
...
‘<div class="set-row set-link" id="setFollowLink"><span>Volgend</span><span class="set-cnt">(’+following.length+’)</span><span class="set-arrow">›</span></div>’+
‘<div class="set-row set-link" id="setMyListLink"><span>Mijn lijst</span><span class="set-cnt">(’+myList.length+’)</span><span class="set-arrow">›</span></div>’+
...
‘<div class="set-row set-link" data-page="privacy"><span>Privacybeleid</span><span class="set-arrow">›</span></div>’+
‘<div class="set-row set-link" data-page="terms"><span>Algemene voorwaarden</span><span class="set-arrow">›</span></div>’+
‘<div class="set-row set-link" data-page="disclaimer"><span>Disclaimer</span><span class="set-arrow">›</span></div>’+
‘<div class="set-row set-link" data-page="deletedata"><span>Data verwijderen</span><span class="set-arrow">›</span></div>’+
‘<div class="set-row set-link" data-page="contact"><span>Contact</span><span class="set-arrow">›</span></div></div>’+
...
‘<div class="set-row set-link" data-page="feedback"><span>App feedback</span><span class="set-arrow">›</span></div></div>’;
main.appendChild(sec);
sec.querySelector(’#themeToggle’).addEventListener(‘click’,function(e){var btn=e.target.closest(’.set-tog’);if(!btn)return;var v=btn.getAttribute(‘data-v’);setSetting(‘theme’,v);sec.querySelectorAll(’#themeToggle .set-tog’).forEach(function(b){b.classList.toggle(‘active’,b.getAttribute(‘data-v’)===v);});if(v===‘system’){var dk=window.matchMedia&&window.matchMedia(’(prefers-color-scheme: dark)’).matches;document.documentElement.setAttribute(‘data-theme’,dk?‘dark’:‘light’);localStorage.removeItem(‘streamgids_theme’);}else{document.documentElement.setAttribute(‘data-theme’,v);try{localStorage.setItem(‘streamgids_theme’,v);}catch(e){}}updateThemeIndicator();});
...
var main=document.getElementById(‘main’);main.innerHTML=’’;
var sec=document.createElement(‘section’);sec.className=‘settings-section’;
var html=’<div class="set-back" id="setBack">← Instellingen</div><div class="set-title">Volgend</div>’;
...
sec.innerHTML=’<div class="set-back" id="setBack">← Instellingen</div><div class="set-title">’+(titles[page]||page)+’</div><div class="set-page-content">’+content+’</div>’;
main.appendChild(sec);
sec.querySelector(’#setBack’).addEventListener(‘click’,function(){renderSettings();});
}
...
updateThemeIndicator();
var themeInd=document.getElementById(‘themeIndicator’);
if(themeInd)themeInd.addEventListener(‘click’,cycleTheme);
var appEl = document.querySelector(’.app’);
...
var appEl    = document.querySelector(’.app’);
if(!ptrEl||!appEl) return;

```
var THRESHOLD = 60;   // px of visual pull needed to trigger refresh
var MAX_PULL  = 110;  // max visual pull (dampened)
var startY    = 0;
var startX    = 0;
var pulling   = false;
var locked    = false; // locked out (horizontal swipe detected)
var refreshing= false;

function isAtTop(){
  return window.scrollY < 2; // tolerance for iOS sub-pixel
}

function resetPTR(animate){
  if(animate){
    appEl.style.transition = 'transform 0.3s cubic-bezier(.2,.8,.4,1)';
    ptrEl.style.transition = 'opacity 0.3s ease';
  }
...
// CRITICAL: passive:false so we can preventDefault to stop iOS rubber-band
document.addEventListener('touchmove', function(e){
  if(!startY || locked || refreshing) return;

  var curY  = e.touches[0].clientY;
  var curX  = e.touches[0].clientX;
  var distY = curY - startY;
  var distX = Math.abs(curX - startX);

  // Horizontal swipe → lock out PTR
  if(!pulling && distX > Math.abs(distY)){
    locked = true;
    return;
  }

  // Only pull downward and only when at top
  if(distY < 5 || !isAtTop()){
    if(pulling) resetPTR(true);
    return;
  }

  // We're pulling! Prevent iOS native overscroll.
  e.preventDefault();
  pulling = true;

  // Dampened pull: starts fast, slows down
  var rawPull = distY;
  var pull = Math.min(rawPull * 0.45, MAX_PULL);
  var progress = Math.min(pull / THRESHOLD, 1);
  var triggered = progress >= 1;

  // Move the entire app down, PTR indicator sits in the gap
  appEl.style.transform = 'translateY(' + pull + 'px)';

  // PTR indicator: positioned fixed at top, fades in
  ptrEl.style.opacity = String(Math.min(progress * 1.3, 1));

  var txt  = ptrEl.querySelector('.ptr-text');
  var spin = ptrEl.querySelector('.ptr-spinner');
  if(txt) txt.textContent = triggered ? 'Loslaten om te vernieuwen' : 'Trek naar beneden';
  if(spin) spin.style.opacity = triggered ? '1' : '0.3';

}, { passive: false }); // <-- must be non-passive for preventDefault

document.addEventListener('touchend', function(){
  if(!pulling || refreshing){ startY=0; startX=0; return; }

  // Check if pull was enough
  var appTransform = appEl.style.transform;
  var match = appTransform.match(/translateY\(([0-9.]+)px\)/);
  var currentPull = match ? parseFloat(match[1]) : 0;

  if(currentPull >= THRESHOLD){
    // Trigger refresh
    refreshing = true;
    var txt = ptrEl.querySelector('.ptr-text');
    if(txt) txt.textContent = 'Vernieuwen...';
    ptrEl.style.opacity = '1';

    // Snap app to indicator height and hold
    appEl.style.transition = 'transform 0.2s ease';
...
// Safety: touchcancel (iOS gesture conflicts, notifications, etc.)
document.addEventListener('touchcancel', function(){
  if(pulling && !refreshing) resetPTR(true);
  startY = 0; startX = 0;
});
```

})();
if(window.matchMedia){window.matchMedia(’(prefers-color-scheme: dark)’).addEventListener(‘change’,function(e){try{var saved=localStorage.getItem(‘streamgids_theme’);if(!saved){document.documentElement.setAttribute(‘data-theme’,e.matches?‘dark’:‘light’);updateThemeIndicator();}}catch(err){}});}
init();
});
