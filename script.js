...
document.addEventListener('DOMContentLoaded',function(){
  ...
  var ptrState={startY:0,startX:0,active:false,triggered:false,locked:false,refreshing:false};
  var PTR_THRESHOLD=70;   /* px pull needed to trigger */
  var PTR_MAX=120;         /* max visual pull distance */
  var PTR_HEIGHT=50;       /* indicator bar height */

  function ptrReset(){
    ptrState.startY=0;ptrState.startX=0;ptrState.active=false;
    ptrState.triggered=false;ptrState.locked=false;
    if(ptrEl){
      ptrEl.style.transition='transform 0.3s ease, opacity 0.3s ease';
      ptrEl.style.transform='translateY(-'+PTR_HEIGHT+'px)';
      ptrEl.style.opacity='0';
      /* Clear transition after animation so dragging feels direct */
      setTimeout(function(){if(ptrEl)ptrEl.style.transition='none';},320);
    }
  }

  function ptrCanStart(){
    /* Check if we're at the very top — allow small tolerance for iOS rubber-band */
    return window.scrollY<=1 && !ptrState.refreshing;
  }

  document.addEventListener('touchstart',function(e){
    if(!ptrCanStart()){ptrState.startY=0;return;}
    ptrState.locked=false;
  ...
  document.addEventListener('touchmove',function(e){
    if(!ptrState.startY||ptrState.locked||ptrState.refreshing)return;
    var curY=e.touches[0].clientY;
    var curX=e.touches[0].clientX;
    var distY=curY-ptrState.startY;
    var distX=Math.abs(curX-ptrState.startX);

    /* If horizontal swipe dominates before PTR activates → lock out */
    if(!ptrState.active&&distX>Math.abs(distY)){ptrState.locked=true;return;}

    /* If scrolled away from top during the gesture, cancel */
    if(window.scrollY>1){
      if(ptrState.active)ptrReset();
      return;
    }

    if(distY>10){
      ptrState.active=true;
      /* Apply rubber-band dampening: the further you pull, the harder it gets */
      var pull=Math.min(distY*0.5,PTR_MAX);
      var progress=Math.min(pull/PTR_THRESHOLD,1);

      if(ptrEl){
        /* Position: starts hidden at -PTR_HEIGHT, slides down */
        ptrEl.style.transform='translateY('+(pull-PTR_HEIGHT)+'px)';
        ptrEl.style.opacity=String(Math.min(progress*1.5,1));
        ptrState.triggered=pull>=PTR_THRESHOLD*0.5;
        var txt=ptrEl.querySelector('.ptr-text');
        var spin=ptrEl.querySelector('.ptr-spinner');
        if(txt)txt.textContent=ptrState.triggered?'Loslaten om te vernieuwen':'Trek naar beneden';
        /* Rotate spinner based on pull progress */
        if(spin)spin.style.animationPlayState=ptrState.triggered?'running':'paused';
      }
    }
  },{ passive:true });

  document.addEventListener('touchend',function(){
    if(!ptrState.active){ptrState.startY=0;ptrState.startX=0;return;}
    if(ptrState.triggered&&!ptrState.refreshing){
      ptrState.refreshing=true;
      if(ptrEl){
        var txt=ptrEl.querySelector('.ptr-text');
        if(txt)txt.textContent='Vernieuwen...';
        /* Hold indicator visible at fixed position during reload */
        ptrEl.style.transition='transform 0.2s ease';
  ...
  /* Safety: if touch gets cancelled (e.g. notification, gesture conflict) */
  document.addEventListener('touchcancel',function(){
    if(ptrState.active)ptrReset();
    ptrState.startY=0;ptrState.startX=0;
  });
