...
function drawHeaderLogo(){
  ...
  ctx.save();
  ctx.beginPath();
  ctx.rect(iX-2,baseline-fontSize*0.62,iW+4,fontSize*0.72);
  ctx.clip();
  ctx.fillStyle=gidsColor;
  ctx.fillText('i',iX,baseline);
  ctx.restore();

  /* Retro TV as the dot of the i — 20% bigger */
  var tvCX=iX+iW/2;
  var dotY=baseline-fontSize*0.9;
...
function selectDate(iso){selectedDate=iso;document.querySelectorAll('.dtab').forEach(function(t){t.classList.toggle('active',t.getAttribute('data-date')===iso);});var a=document.querySelector('.dtab.active');if(a)a.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});window.scrollTo({top:0,behavior:'smooth'});renderMain();}

/* ── Navigate to next/previous day ── */
function navigateDay(direction){
  if(!allDays.length)return;
  var curIdx=allDays.indexOf(selectedDate);
  if(curIdx===-1)return;
  var newIdx=curIdx+direction;
  if(newIdx<0||newIdx>=allDays.length)return;
  selectDate(allDays[newIdx]);
}
...
function renderSettingsPage(page){
  ...
  if(page==='privacy'){
    ...
    '<ul><li>Je mijn-lijstitems (opgeslagen titels)</li><li>Gevolgde titels</li><li>Themavoorkeur (licht, donker, systeem)</li><li>Posterweergave-instelling</li><li>Meldingsvoorkeuren</li><li>Gecachte content (voor snellere laadtijden)</li></ul>'+
    ...
    '<ul><li><strong>The Movie Database (TMDb)</strong> — voor filminformatie, posters en beoordelingen.</li><li><strong>Google Sheets</strong> — voor handmatig samengestelde content zoals kijktips en sportevenementen.</li><li><strong>football-data.org en Jolpica API</strong> — voor live sportuitslagen.</li></ul>'+
    ...
    '<h3>3. Cookies en tracking</h3><p>De App maakt geen gebruik van cookies, advertentie-trackers of analytics-scripts van derden.</p>'+
    '<h3>4. Gegevens van kinderen</h3><p>De App verzamelt niet bewust gegevens van kinderen jonger dan 16 jaar.</p>'+
    '<h3>5. Je rechten</h3><p>Alle persoonlijke gegevens worden lokaal opgeslagen. Je kunt op elk moment alles verwijderen via Instellingen &gt; Data verwijderen.</p>'+
    '<h3>6. Wijzigingen</h3><p>Dit privacybeleid kan van tijd tot tijd worden bijgewerkt.</p>'+
    '<h3>7. Contact</h3><p><a href="mailto:info@streamgidsapp.nl">info@streamgidsapp.nl</a></p>';
  }
  else if(page==='terms'){
    content='<p><strong>Laatst bijgewerkt:</strong> maart 2025</p>'+
    '<p>Door het gebruik van StreamGids ga je akkoord met deze voorwaarden.</p>'+
    '<h3>1. Algemeen</h3><p>De App is ontwikkeld door Rex Raba en biedt een overzicht van beschikbare content op streamingdiensten in Nederland.</p>'+
    '<h3>2. Gebruik</h3><p>Je mag de App gebruiken voor persoonlijk, niet-commercieel gebruik.</p>'+
    '<h3>3. Content en beschikbaarheid</h3><p>StreamGids garandeert niet dat informatie altijd correct of actueel is.</p>'+
    '<h3>4. Intellectueel eigendom</h3><p>Alle rechten op het ontwerp en de code berusten bij Rex Raba.</p>'+
    '<h3>5. Aansprakelijkheid</h3><p>De App wordt aangeboden "as is", zonder garantie.</p>'+
    '<h3>6. Toepasselijk recht</h3><p>Nederlands recht is van toepassing.</p>'+
    '<h3>7. Contact</h3><p><a href="mailto:info@streamgidsapp.nl">info@streamgidsapp.nl</a></p>';
  }
  else if(page==='disclaimer'){
    content='<p><strong>Laatst bijgewerkt:</strong> maart 2025</p>'+
    '<h3>Geen officiële affiliatie</h3><p>StreamGids is onafhankelijk en niet gelieerd aan enige streamingdienst.</p>'+
    '<h3>Contentgegevens</h3><p>Dit product maakt gebruik van de TMDb-API maar wordt niet goedgekeurd of gecertificeerd door TMDb.</p>'+
    '<h3>Contact</h3><p><a href="mailto:info@streamgidsapp.nl">info@streamgidsapp.nl</a></p>';
  }
  else if(page==='deletedata'){
    content='<h3>Jouw gegevens</h3><p>StreamGids slaat alle gegevens uitsluitend lokaal op.</p>'+
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  (function(){
    var sheet=document.getElementById('sheetEl');
    if(!sheet)return;
    ...
    sheet.addEventListener('touchmove',function(e){
      ...
      if(dy<0){dy=0;}
      /* If sheet was scrolled and user scrolls up, cancel drag */
      if(sheet.scrollTop>0){isDragging=false;sheet.style.transform='';return;}
    ...
    sheet.addEventListener('touchend',function(){
      ...
      if(dy>100){
        ...
        sheet.style.transform='';
        sheet.style.opacity='';
        setTimeout(function(){sheet.style.transition='';},250);
      }
    });
  })();
  ...
  (function(){
    ...
    if(!mainEl)return;
    var swStartX=0,swStartY=0,swLocked=false,swDirection=null;
    var SWIPE_THRESHOLD=50;
    var ANGLE_LOCK=25; /* degrees — if vertical movement exceeds this ratio, don't treat as horizontal swipe */
    ...
    mainEl.addEventListener('touchmove',function(e){
      ...
      if(!swDirection&&(Math.abs(dx)>10||Math.abs(dy)>10)){
        if(Math.abs(dy)>Math.abs(dx)){
          /* Vertical scroll — lock out horizontal swipe */
          swLocked=true;
          return;
        }
        swDirection='horizontal';
      }
    },{passive:true});
    mainEl.addEventListener('touchend',function(e){
      if(swLocked||swDirection!=='horizontal')return;
