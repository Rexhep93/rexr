...
function crowHtml(item){
  ...
  var epBadge='';
  if(item._type==='tv'){var ep=item._epInfo;var hasSeason=!!item._season;var hasEpInfo=!!(ep&&ep.s);var isGS=item._source==='googlesheet';if(hasEpInfo){var totalEps=ep.seasonEpisodeCount||null;var completeDrop=ep.isCompleteDrop||false;var fullSeasonAvailable=ep.airedEpisodes&&ep.seasonEpisodeCount&&ep.airedEpisodes>=ep.seasonEpisodeCount;if(ep.e===1||completeDrop||fullSeasonAvailable){epBadge='<div class=\"crow-ep-badge season\">Nieuw seizoen (S'+ep.s+')</div>';}else{{if(totalEps){epBadge='<div class="crow-ep-badge">Nieuwe aflevering '+ep.e+' van '+totalEps+'</div>';}else{epBadge='<div class="crow-ep-badge">Nieuwe aflevering</div>';}epBadge+='<div class="crow-ep-badge season">Seizoen '+ep.s+'</div>';}}else if(hasSeason){epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+item._season+')</div>';}else if(isGS){epBadge='<div class="crow-ep-badge">Nieuwe aflevering</div>';}}
