...
function canonicalProviderLogo(key){
  /* Return original brand logo by key, preferring the main brand (not Kids/ads variants) */
  var canonKey=(key||'').toLowerCase();
  var fallback=null;
  for(var pid in TMDB_NL_PROVIDERS){
    var pr=TMDB_NL_PROVIDERS[pid];
    if(pr&&pr.name&&providerKey(pr.name)===canonKey&&pr.logo){
      var raw=(pr.rawName||'').toLowerCase();
      /* Skip variant names like "Netflix Kids", "Netflix basic with Ads" */
...
function capitalizeProvider(n){
  /* Ensure streamer name uses proper capitalization */
...
function updateThemeIndicator(){
  ...
  if(themeVal==='system'){
    ...
    if(isDark){
      el.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg><span>Auto</span>';
    }else{
      el.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg><span>Auto</span>';
    }
  }else{
    el.innerHTML='';
  }
...
function mergeItems(wm,tm,gs){
  ...
  /* Google Sheet: TV series added for weekly episodes, BUT skip if API
     already has the exact same title+provider (any date) to prevent duplicates
     like "Scrubs" showing on two consecutive days. */
  (gs||[]).forEach(function(i){
    ...
    if(i._type==='tv'){
      ...
      /* If API already has this title+provider on ANY date → skip (prevents wrong-date duplicates) */
      var apiHasTitleProvider=false;
      for(var dk in apiDateKeys){if(dk.indexOf(tl+'|'+i._key+'|')===0){apiHasTitleProvider=true;break;}}
      if(apiHasTitleProvider)return;
  ...
  if(item._type==='live'){
    ...
    if(hasEpInfo){
      ...
      if(completeDrop&&totalEps&&airedEps>=totalEps){
        /* All episodes dropped at once → "Nieuw seizoen (S3)" */
        epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';
      }else if(isFirstEp||hasSeason){
        /* New season starting */
        epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';
        if(totalEps){
          epBadge+='<div class="crow-ep-badge">Nieuwe aflevering '+ep.e+' van '+totalEps+' Seizoen '+ep.s+'</div>';
        }else{
          epBadge+='<div class="crow-ep-badge">Nieuwe aflevering Seizoen '+ep.s+'</div>';
        }
      }else{
