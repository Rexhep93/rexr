...
function enrichMyListDates(callback){
  ...
  gsNoTmdb.forEach(function(item){
    var liveItems=allItems.filter(function(i){return i._source==='googlesheet'&&i._type==='tv'&&i.title===item.title;}).sort(function(a,b){return a._date.localeCompare(b._date);});
    var current=liveItems.filter(function(i){return i._date<=today;}).pop();
    var next=liveItems.filter(function(i){return i._date>today;})[0];
  ...
  if(!tvItems.length&&!gsNoTmdb.length){callback();return;}
  callback();
}
