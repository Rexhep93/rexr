...
function enrichMyListDates(callback){
  var today=todayISO();
  function epStr(epInfo){return epInfo?'S'+String(epInfo.s).padStart(2,'0')+'E'+String(epInfo.e).padStart(2,'0'):null;}
  myList.filter(function(f){return f._type==='tv'&&f.tmdb_id;}).forEach(function(item){
    ...
    if(isGS){
      ...
      item._lastEp=current?(current._gsEp||epStr(current._epInfo)):null;
      ...
      if(liveItem&&liveItem._date){
        if(liveItem._date>today){
          ...
          item._nextEp=epStr(liveItem._epInfo);
          item._date=today;item._lastEp=null;
        } else {
          item._date=liveItem._date;
          item._lastEp=epStr(liveItem._epInfo);
  ...
  myList.filter(function(f){return f._type==='tv'&&!f.tmdb_id;}).forEach(function(item){
    ...
    var current=null;var nextItem=null;
    gsAll.forEach(function(i){if(i._date<=today)current=i;else if(!nextItem)nextItem=i;});
    if(current){item._date=current._date;item._lastEp=current._gsEp||null;}
