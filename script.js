...
function cycleTheme(){
  ...
  refreshLogo(); /* WIJZIGING #2: Logo herladen bij cycleTheme */
}
...
function enrichEpisodeDetails(){
  ...
  if(!tvItems.length)return Promise.resolve();
  var idx=0;
  function next(){
...
function renderSettings(){
  ...
  main.appendChild(sec);
  sec.querySelector('#themeToggle').addEventListener('click',function(e){var btn=e.target.closest('.set-tog');if(!btn)return;var v=btn.getAttribute('data-v');setSetting('theme',v);sec.querySelectorAll('#themeToggle .set-tog').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-v')===v);});if(v==='system'){var dk=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches;document.documentElement.setAttribute('data-theme',dk?'dark':'light');localStorage.removeItem('streamgids_theme');}else{document.documentElement.setAttribute('data-theme',v);try{localStorage.setItem('streamgids_theme',v);}catch(e){}}updateThemeIndicator();refreshLogo(); /* WIJZIGING #3: Logo herladen bij thema-toggle in Settings */ });
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  /* Draw brand logo after fonts load */ /* WIJZIGING #1: Logo tekenen bij opstart */
  if(document.fonts&&document.fonts.ready){document.fonts.ready.then(function(){refreshLogo();});}else{setTimeout(function(){refreshLogo();},100);}
  ...
  if(window.matchMedia){window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change',function(e){try{var saved=localStorage.getItem('streamgids_theme');if(!saved){document.documentElement.setAttribute('data-theme',e.matches?'dark':'light');updateThemeIndicator();refreshLogo();}}catch(err){}});} /* WIJZIGING #4: Logo herladen bij systeem-thema wijziging */
  init();
});
