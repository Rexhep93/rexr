...
/* ── MY LIST ── */
function renderMyListItem(f,mode,extraClass){
  var poster=f.img||'';
  ...
  if(sn&&sn!=='Onbekend'){
    if(provLogo){
      provHtml='<div class="crow-also-on ml-prov"><img src="'+provLogo+'" alt="'+sn+'" onerror="this.parentElement.style.display=\'none\'" loading="lazy"><span class="crow-also-on-label">'+sn+'</span></div>';
    }else{
      provHtml='<div class="crow-also-on ml-prov"><span class="crow-also-on-label">'+sn+'</span></div>';
    }
  }
  /* ── Episode / season badges ── */
  var epBadgesHtml='';
  if(!isMovie){
    var epSrc=(mode==='next'&&f._nextEp)?f._nextEp:null;
    if(epSrc){
  ...
  return '<div class="crow fav-crow'+(extraClass?' '+extraClass:'')+'" data-id="'+f.id+'">'
    +'<div class="crow-poster">'+(poster?'<img src="'+poster+'" alt="" loading="lazy">':'<div class="crow-fb">'+f.title+'</div>')+'</div>'
    +'<div class="crow-info">'
      +'<div class="crow-title">'+f.title+'</div>'
      +(epBadgesHtml?'<div class="crow-ep-row">'+epBadgesHtml+'</div>':'')
      +provHtml
    +'</div>'
    +typeBadge
    +'<button class="mylist-remove-btn" data-id="'+f.id+'" aria-label="Verwijder">✕</button>'
  +'</div>';
}
