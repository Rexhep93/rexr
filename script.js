...
function enrichMyListDates(callback){
  ...
  tvItems.forEach(function(item){
    ...
    if(isGS){
      ...
      var current=null;var next=null;
      gsAll.forEach(function(i){if(i._date<=today)current=i;else if(!next)next=i;});
      if(current){item._date=current._date;item._lastEp=current._gsEp||null;}
      else{item._date=today;item._lastEp=null;}
      if(next){item._nextDate=next._date;item._nextEp=next._gsEp||null;}
      else{item._nextDate=null;item._nextEp=null;}
    } else {
  ...
  var gsNoTmdb=myList.filter(function(f){return f._type==='tv'&&!f.tmdb_id&&f._source==='googlesheet';});
  gsNoTmdb.forEach(function(item){
