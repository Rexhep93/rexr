...
function goHome(){activeTab='stream';selectedDate=todayISO();typeFilter='all';svcFilter='all';streamType='all';streamSubFilter=null;svcBarVisible=false;document.querySelectorAll('.bnav').forEach(function(n){n.classList.toggle('active',n.getAttribute('data-f')==='stream');});var dtWrap=document.querySelector('.date-tabs-wrap');var sb=document.getElementById('svcBar');var fb=document.getElementById('filterBar');if(dtWrap)dtWrap.style.display='';if(sb)sb.style.display='none';if(fb)fb.style.display='';updateSvcClearFloat();buildDateTabs();buildFilterChips();renderMain();window.scrollTo({top:0,behavior:'smooth'});}
...
function updateSwipeHints(){
  var allD=allDays;if(!allD.length)return;
  var idx=allD.indexOf(selectedDate);
  var leftHint=document.getElementById('swipeHintLeft');
  var rightHint=document.getElementById('swipeHintRight');
  if(leftHint)leftHint.style.opacity=idx>0?'1':'0';
  if(rightHint)rightHint.style.opacity=idx<allD.length-1?'1':'0';
}
...
function updateSvcClearFloat(){
  ...
  if(!btn)return;
  var hasFilter=activeTab==='stream'&&(svcFilter!=='all'||streamType!=='all'||streamSubFilter!==null);
  if(hasFilter){
    btn.style.display='flex';
    var parts=[];
    if(streamType==='movie')parts.push('Films');
    else if(streamType==='tv')parts.push('Series');
    if(streamSubFilter==='seasons')parts.push('Nieuwe seizoenen');
    else if(streamSubFilter==='episodes')parts.push('Nieuwe afleveringen');
    if(svcFilter!=='all')parts.push(capitalizeProvider(svcFilter));
    if(label)label.textContent=(parts.length?parts.join(' · ')+' ':'')+'✕';
  }else{
...
function handleFilterClick(sf){
  ...
  haptic('light');
  if(sf==='back'){streamType='all';streamSubFilter=null;updateSvcClearFloat();buildFilterChips();renderStreamView();return;}
  if(streamType==='all'){
    ...
    updateSvcClearFloat();buildFilterChips();renderStreamView();
  }
}
...
function renderStreamView(){var dtWrap=document.querySelector('.date-tabs-wrap');if(dtWrap)dtWrap.style.display='';renderMain();}
...
function crowHtml(item){
  ...
  var liveCls='';
  var metaL=tl+' - '+sn;
  return '<div class="crow'+liveCls+'" data-id="'+sid+'"><div class="crow-poster'+(poster?'':' crow-poster-skeleton')+'">'+(poster?'<img src="'+poster+'" alt="'+title+'" loading="lazy" onerror="this.parentElement.classList.remove(\'crow-poster-skeleton\');this.parentElement.innerHTML=\'<div class=crow-fb>'+title.replace(/'/g,'').replace(/"/g,'')+'</div>\'">':'<div class="crow-fb" id="fb-'+sid+'">'+title+'</div>')+'</div><div class="crow-info"><div class="crow-title">'+title+'</div>'+epBadge+'<div class="crow-meta">'+metaL+'</div>'+alsoOnHtml+'</div><div class="crow-badge '+cls+'">'+tl+'</div></div>';
}
...
function setType(f){
  ...
  var dtWrap=document.querySelector('.date-tabs-wrap');var sb=document.getElementById('svcBar');var fb=document.getElementById('filterBar');
  if(f==='stream'){streamType='all';streamSubFilter=null;svcBarVisible=false;if(dtWrap)dtWrap.style.display='';sb.style.display='none';fb.style.display='';updateSvcClearFloat();buildFilterChips();renderMain();}
  else if(f==='top10'){if(dtWrap)dtWrap.style.display='none';sb.style.display='none';fb.style.display='none';updateSvcClearFloat();renderTop10();}
  else if(f==='kijktip'){if(dtWrap)dtWrap.style.display='none';sb.style.display='none';fb.style.display='none';updateSvcClearFloat();renderKijktip();}
  else if(f==='search'){if(dtWrap)dtWrap.style.display='none';sb.style.display='none';fb.style.display='none';updateSvcClearFloat();renderSearch();}
  else if(f==='settings'){if(dtWrap)dtWrap.style.display='none';sb.style.display='none';fb.style.display='none';updateSvcClearFloat();renderSettings();}
  else if(f==='mylist'){if(dtWrap)dtWrap.style.display='none';sb.style.display='none';fb.style.display='none';updateSvcClearFloat();renderMyList();}
}
...
function openModal(rawId){
  ...
  si.innerHTML='<div class="sheet-header"><div class="sp'+spClass+'">'+(initPoster?'<img src="'+initPoster+'" alt="">':'')+'</div><div class="sinf"><div class="ssvc" style="color:'+color+'">'+svcName+'</div><div class="stitle">'+title+'</div><div class="smeta">'+(_type==='movie'?'Film':'Serie')+' - laden...</div></div></div>';
  ...
  if(resolvedTmdbId){chain=chain.then(function(){var cKey='tmdb-'+_type+'-'+resolvedTmdbId;if(detailCache[cKey])return detailCache[cKey];var path=_type==='movie'?'/movie/'+resolvedTmdbId:'/tv/'+resolvedTmdbId;return Promise.all([tmdb(path,{language:'nl-NL'}),tmdb(path+'/watch/providers'),tmdb(path+'/external_ids').catch(function(){return{};}),tmdb(path+'/credits',{language:'nl-NL'}).catch(function(){return{cast:[]};}),tmdb(path+'/similar',{language:'nl-NL',page:1}).catch(function(){return{results:[]};})]).then(function(r){detailCache[cKey]={det:r[0],prov:r[1],extIds:r[2],credits:r[3],similar:r[4]};return detailCache[cKey];}).catch(function(){detailCache[cKey]=null;return null;});}).then(function(cached){if(cached&&cached.det){var d=cached.det;fullTitle=d.title||d.name||title;year=(d.release_date||d.first_air_date||'').slice(0,4);runtime=d.runtime?d.runtime+' min':'';tmdbRating=d.vote_average||tmdbRating;genres=(d.genres||[]).slice(0,4).map(function(g){return '<span class="stag">'+g.name+'</span>';}).join('');if(d.overview)rawOverview=d.overview;if(_type==='tv'&&d.number_of_seasons)seasonInfo=d.number_of_seasons+' seizoen'+(d.number_of_seasons>1?'en':'');if(d.poster_path){posterUrl=TMDB_IMG+d.poster_path;imgCache[item.id]=posterUrl;item.img=posterUrl;}var nlProv=cached.prov&&cached.prov.results&&cached.prov.results.NL;if(nlProv&&nlProv.link)watchLink=nlProv.link;imdbId=cached.extIds&&cached.extIds.imdb_id||null;
    ...
    /* ── Similar titles (improved: franchise > genre match, NL-only) ── */
    if(cached.similar&&similarRow){
      var det=cached&&cached.det||null;
      var origTitle=(det&&(det.title||det.name)||title||'').toLowerCase();
      var origGenres=(det&&det.genre_ids)||((det&&det.genres||[]).map(function(g){return g.id;}));
      var origYear=parseInt((det&&(det.release_date||det.first_air_date)||'').slice(0,4))||0;
      var rawSims=(cached.similar&&cached.similar.results||[]).filter(function(s){return s.poster_path;});
      /* Score each result */
      rawSims=rawSims.map(function(s){
        var score=0;
        var sTitle=(s.title||s.name||'').toLowerCase();
        /* Franchise/series match: titles share significant words */
        var origWords=origTitle.split(/\s+/).filter(function(w){return w.length>3;});
        var matchWords=origWords.filter(function(w){return sTitle.indexOf(w)!==-1;});
        if(matchWords.length>=1)score+=50*matchWords.length;
        /* Same year range (+/- 5 years): prefer similar era */
        var sYear=parseInt((s.release_date||s.first_air_date||'').slice(0,4))||0;
        if(origYear&&sYear&&Math.abs(sYear-origYear)<=5)score+=8;
        /* Genre overlap */
        var sGenres=s.genre_ids||[];
        var overlap=origGenres.filter(function(g){return sGenres.indexOf(g)!==-1;}).length;
        score+=overlap*5;
        /* Popularity boost */
        score+=(s.vote_average||0)*0.5;
        /* Penalize low vote count (likely obscure) */
        if((s.vote_count||0)<50)score-=15;
        s._simScore=score;
        return s;
      });
      rawSims.sort(function(a,b){return b._simScore-a._simScore;});
      var sims=rawSims.slice(0,10);
      if(sims.length){
