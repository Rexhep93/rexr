  ...
  function next(){
    ...
    return Promise.all(batch.map(function(item){
      ...
      return p.then(function(d){
        ...
        /* Voor TV: haal seizoen-info op zodat _epInfo beschikbaar is */
        if(item._type==='tv'&&item.tmdb_id&&(d.next_episode_to_air||d.last_episode_to_air)){
          var candidates=[];
          if(d.next_episode_to_air)candidates.push({date:d.next_episode_to_air.air_date,s:d.next_episode_to_air.season_number,e:d.next_episode_to_air.episode_number,name:d.next_episode_to_air.name||''});
          if(d.last_episode_to_air)candidates.push({date:d.last_episode_to_air.air_date,s:d.last_episode_to_air.season_number,e:d.last_episode_to_air.episode_number,name:d.last_episode_to_air.name||''});
          ...
          if(best){
            return tmdb('/tv/'+item.tmdb_id+'/season/'+best.s,{language:'nl-NL'}).then(function(seasonDetail){
              ...
              var firstEpDate=(eps.length>0&&eps[0].air_date)?eps[0].air_date:null;
              item._epInfo={s:best.s,e:best.e,name:best.name,seasonEpisodeCount:eps.length,airedEpisodes:aired,isCompleteDrop:false,seasonFirstAirDate:firstEpDate};
            }).catch(function(){});
          }
        }
      }).catch(function(){});
...
function mergeItems(wm,tm,gs){
  ...
  /* Google Sheet: TV series added for weekly episodes, BUT skip if API
     already has the same title+provider on a NEARBY date (±1 day).
     That means it's the same episode with a slightly off date (e.g. API
     says March 4, GS says March 3). Different weeks must still show. */
  (gs||[]).forEach(function(i){
    ...
    if(i._type==='tv'){
      ...
      /* Check if API has same title+provider within ±1 day → same episode, skip */
      var gsTime=new Date(i._date+'T12:00:00').getTime();
      var nearbyDup=false;
      for(var dk in apiDateKeys){
        if(dk.indexOf(tl+'|'+i._key+'|')!==0)continue;
        var apiDateStr=dk.split('|')[2];
        var apiTime=new Date(apiDateStr+'T12:00:00').getTime();
        if(Math.abs(gsTime-apiTime)<=86400000){nearbyDup=true;break;}
      }
      if(nearbyDup)return;
...
function crowHtml(item){
  ...
  if(item._type==='live'){
    ...
    if(hasEpInfo){
      ...
      /* Nieuw seizoen als:
         - ep.e === 1 (eerste aflevering)
         - OF eerste aflevering van het seizoen is recent (binnen 14 dagen) */
      var recentSeasonStart=false;
      if(ep.seasonFirstAirDate){
        var daysDiff=Math.abs(new Date(todayISO()+'T12:00:00')-new Date(ep.seasonFirstAirDate+'T12:00:00'))/86400000;
        recentSeasonStart=daysDiff<=14;
      }
      var isNewSeason=isFirstEp||recentSeasonStart;
      if(isNewSeason){
