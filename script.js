...
function isInMyList(id){
  /* Exact id match */
  if(myList.some(function(f){return f.id===id;}))return true;
  /* For series/live: match by tmdb_id or by normalized title */
  var item=allItems.find(function(i){return String(i.id)===String(id);});
  if(!item)return false;
  return myList.some(function(f){
    if(item.tmdb_id&&f.tmdb_id&&item.tmdb_id===f.tmdb_id)return true;
    if(item._type==='tv'&&f._type==='tv'&&item.title&&f.title&&item.title.toLowerCase().trim()===f.title.toLowerCase().trim())return true;
    return false;
  });
}
function getMyListCanonicalId(item){
  /* Return existing list entry id if same series already exists */
  if(!item)return null;
  for(var i=0;i<myList.length;i++){
    var f=myList[i];
    if(f.id===item.id)return f.id;
    if(item.tmdb_id&&f.tmdb_id&&item.tmdb_id===f.tmdb_id)return f.id;
    if(item._type==='tv'&&f._type==='tv'&&item.title&&f.title&&item.title.toLowerCase().trim()===f.title.toLowerCase().trim())return f.id;
  }
  return null;
}
...
function updateThemeToggleBtn(){
  ...
  var v=getSetting('theme','auto');
  btn.innerHTML=THEME_ICONS[v]||THEME_ICONS.auto;
  btn.setAttribute('data-theme-val',v);
  btn.title=v==='auto'?'Auto (systeem)':v==='light'?'Licht':'Donker';
}
function cycleTheme(){
  var cur=getSetting('theme','auto');
  ...
  /* Also sync settings page toggle if open */
  document.querySelectorAll('#themeToggle .set-tog').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-v')===next);});
}
  ...
  function next(){
    ...
    return Promise.all(batch.map(function(item){
      ...
      return p.then(function(d){
        ...
        /* Silently skip invalid TMDB IDs (404 etc) */
        if(item.tmdb_id&&String(e&&e.message||'').indexOf('404')!==-1){item.tmdb_id=null;}
      });
    })).then(next);
  }
...
function mergeItems(wm,tm,gs){
  ...
  function addItem(i){
    ...
    var tk=tl+'|'+i._key+'|'+i._date;
    if(seen[tk])return;seen[tk]=true;
    /* titleProvMap key includes date: same title on different dates are separate entries */
    var tdk=tl+'|'+i._date;
    if(titleProvMap[tdk]){
      var existing=titleProvMap[tdk];
    ...
    titleProvMap[tdk]={item:i,providers:[i._key]};result.push(i);
  }
  ...
  (gs||[]).forEach(function(i){
    ...
    if(i._type==='tv'){
      ...
      var tdk=tl+'|'+i._date;
      if(titleProvMap[tdk]){var ex=titleProvMap[tdk].item;if(!i.img&&ex.img){i.img=ex.img;imgCache[i.id]=ex.img;}if(!i.overview&&ex.overview)i.overview=ex.overview;if(!i.tmdb_id&&ex.tmdb_id)i.tmdb_id=ex.tmdb_id;}
      result.push(i);
    }else{addItem(i);}
  });
  return result;
...
function fetchSportSheet(){
  ...
  function parseCsv(csv){
    ...
    for(var i=1;i<lines.length;i++){
      ...
      if(streamer&&!SPORT_STREAMERS[streamer]){var mp=matchProvider(streamer);if(mp)SPORT_STREAMERS[streamer]={bg:mp.color};else SPORT_STREAMERS[streamer]={bg:'rgba(100,100,100,0.85)'};}
      items.push({id:'gs-sport-'+i+'-'+titel.replace(/\s/g,'-').slice(0,20),title:titel,subtitle:sport||'Live Event',sport:sport||'Live Event',streamer:streamer||'Streaming',date:d,venue:'',isLive:false});
    }
    console.log('[StreamGids] Sport Sheet: '+items.length+' events');
    scSet(ck,items);return items;
  }
  function fetchUrl(url){return fetch(url).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.text();}).then(parseCsv);}
  var proxyUrl='https://corsproxy.io/?url='+encodeURIComponent(GOOGLE_SHEET_SPORT_URL);
  return fetchUrl(GOOGLE_SHEET_SPORT_URL).catch(function(){return fetchUrl(proxyUrl);}).catch(function(e){console.warn('[StreamGids] Sport Sheet fout:',e);return[];});
}
...
function renderSettings(){
  var main=document.getElementById('main');main.innerHTML='';
  var sec=document.createElement('section');sec.className='settings-section';
  var themeVal=getSetting('theme','auto');var pSize=getSetting('posterSize','compact');
  ...
  '<div class="set-row"><span>Thema</span><div class="set-toggle-group" id="themeToggle"><button class="set-tog'+(themeVal==='light'?' active':'')+'" data-v="light">Licht</button><button class="set-tog'+(themeVal==='dark'?' active':'')+'" data-v="dark">Donker</button><button class="set-tog'+(themeVal==='auto'?' active':'')+'" data-v="auto">Auto</button></div></div>'+
  ...
  '<div class="set-row"><span>Nieuwe releases volgend</span><label class="set-switch"><input type="checkbox" id="notifRel"'+(notifRel?' checked':'')+'><span class="set-slider"></span></label></div>'+
  '<div class="set-row"><span>Mijn lijst updates</span><label class="set-switch"><input type="checkbox" id="notifFav"'+(notifFav?' checked':'')+'><span class="set-slider"></span></label></div></div>'+
  ...
  main.appendChild(sec);
  sec.querySelector('#themeToggle').addEventListener('click',function(e){var btn=e.target.closest('.set-tog');if(!btn)return;var v=btn.getAttribute('data-v');setSetting('theme',v);sec.querySelectorAll('#themeToggle .set-tog').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-v')===v);});if(v==='auto'){var dk=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches;document.documentElement.setAttribute('data-theme',dk?'dark':'light');localStorage.removeItem('streamgids_theme');}else{document.documentElement.setAttribute('data-theme',v);try{localStorage.setItem('streamgids_theme',v);}catch(e){}}updateThemeToggleBtn();});
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(themeToggleBtn){updateThemeToggleBtn();themeToggleBtn.addEventListener('click',cycleTheme);}
  ...
  if(myListBtn)myListBtn.addEventListener('click',function(){
    if(!currentModalItem)return;
    if(isInMyList(currentModalItem.id)){
      var existingId=getMyListCanonicalId(currentModalItem)||currentModalItem.id;
      removeFromMyList(existingId);showToast('Verwijderd uit jouw lijst');
    }
