...
/* ══════════════════════════════════════════════════════════
   SPIN & KIJK (enhanced with streamer filter + popular titles)
   ══════════════════════════════════════════════════════════ */
var spinExcluded=[];var spinFilter='all';var spinCache=null;var spinCurrent=null;
var spinSelectedStreamers=[]; /* empty = all streamers */
var spinProviderCache={}; /* cache provider info per tmdb id */
...
function fetchSpinPool(){
  ...
  return Promise.all([
    ...
    /* Popular (non-trending) titles with high vote counts */
    tmdb('/movie/popular',{language:'nl-NL',region:'NL',page:1}).catch(function(){return{results:[]};}),
    tmdb('/movie/popular',{language:'nl-NL',region:'NL',page:2}).catch(function(){return{results:[]};}),
    tmdb('/tv/popular',{language:'nl-NL',page:1}).catch(function(){return{results:[]};}),
    tmdb('/tv/popular',{language:'nl-NL',page:2}).catch(function(){return{results:[]};}),
    tmdb('/movie/top_rated',{language:'nl-NL',region:'NL',page:1}).catch(function(){return{results:[]};}),
    tmdb('/tv/top_rated',{language:'nl-NL',page:1}).catch(function(){return{results:[]};})
  ]).then(function(pages){
    var trending=[];var popular=[];var seen={};
    /* First 5 are trending */
    for(var t=0;t<5;t++){(pages[t].results||[]).forEach(function(x){if(!seen[x.id]){seen[x.id]=true;x._spinSource='trending';trending.push(x);}});}
    /* Rest are popular/top_rated */
    for(var p=5;p<pages.length;p++){(pages[p].results||[]).forEach(function(x){if(!seen[x.id]){seen[x.id]=true;/* Assign media_type if missing */if(!x.media_type){x.media_type=x.title?'movie':'tv';}x._spinSource='popular';popular.push(x);}});}
    /* Filter popular to high vote count only */
    popular=popular.filter(function(x){return(x.vote_count||0)>=200;});
    spinCache={trending:trending,popular:popular};
    return spinCache;
  });
}
...
function getAvailableSpinStreamers(){
  /* Get all known NL streamers for the filter dropdown */
  var streamers=[];
...
function doSpin(sec){
  ...
  fetchSpinPool().then(function(pool){
    /* Decide: 70% trending, 30% popular */
    var useTrending=Math.random()<0.7;
    var sourcePool=useTrending?pool.trending:pool.popular;
    /* Fallback if chosen pool is empty */
    if(!sourcePool.length)sourcePool=pool.trending.concat(pool.popular);

    /* Filter by type */
    var filtered=sourcePool;
    if(spinFilter==='movie')filtered=sourcePool.filter(function(i){return i.media_type==='movie'||(!i.name&&i.title);});
    if(spinFilter==='tv')filtered=sourcePool.filter(function(i){return i.media_type==='tv'||(i.name&&!i.title);});
    ...
    /* Pick a candidate and check NL streaming */
    var attempts=0;var maxAttempts=Math.min(filtered.length,15);
    ...
    function tryPick(){
      if(attempts>=maxAttempts){
        ...
        var rb=sec.querySelector('#spinResetBtn');if(rb)rb.addEventListener('click',function(){spinExcluded=[];spinSelectedStreamers=[];updateSpinExInfo();renderSpinKijk();});
        return;
      }

      var pick=weightedRandom(filtered);
      /* Remove from filtered to avoid picking same one */
      filtered=filtered.filter(function(x){return x.id!==pick.id;});
      attempts++;

      var isTV=pick.media_type==='tv'||(pick.name&&!pick.title);
      var type=isTV?'tv':'movie';

      /* Check NL streaming - use cache if available */
      ...
      provPromise.then(function(pd){
        ...
        if(!nlFlat.length){
          /* Not available in NL - try another */
          tryPick();return;
        }

        /* Check streamer filter */
        if(spinSelectedStreamers.length>0){
        ...
        /* Build the watch link from provider info, NOT TMDB link */
        var nlLink=null;
        /* Get provider-specific search link */
        var svc=streamer.toLowerCase();
        var pickTitle=pick.title||pick.name||'';
        if(svc.includes('netflix'))nlLink='https://www.netflix.com/search?q='+encodeURIComponent(pickTitle);
        else if(svc.includes('prime'))nlLink='https://www.amazon.nl/s?k='+encodeURIComponent(pickTitle);
        else if(svc.includes('disney'))nlLink='https://www.disneyplus.com/search/'+encodeURIComponent(pickTitle);
        else if(svc.includes('apple'))nlLink='https://tv.apple.com/';
        else if(svc.includes('max'))nlLink='https://www.max.com/';
        else if(svc.includes('sky'))nlLink='https://www.skyshowtime.com/';
        else if(svc.includes('videoland'))nlLink='https://www.videoland.com/';
        else if(svc.includes('path'))nlLink='https://www.pathe-thuis.nl/';
        else if(svc.includes('npo'))nlLink='https://npo.nl/';
        else if(svc.includes('paramount'))nlLink='https://www.paramountplus.com/nl/';
        else if(svc.includes('discovery'))nlLink='https://www.discoveryplus.com/nl/';
        else if(svc.includes('viaplay'))nlLink='https://viaplay.nl/';
        else if(svc.includes('mubi'))nlLink='https://mubi.com/';
        else{
          /* Fallback to TMDB link */
          nlLink=pd&&pd.results&&pd.results.NL&&pd.results.NL.link||null;
        }
