...
function drawHeaderLogo(){
  ...
  wrapper.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="-40 -1051 4672 1444" style="height:2.4em;width:auto;display:block;">
...
/* ── MY LIST ── */
function renderMyListItem(f,mode){var poster=f.img||'';var tl=f._type==='movie'?'Film':'Serie';var sn=capitalizeProvider(f._src&&f._src.name||'');var epLabel='';if(f._type==='tv'){if(mode==='next'&&f._nextEp){epLabel=' · '+f._nextEp;}else if(f._lastEp){epLabel=' · t/m '+f._lastEp;}}return '<div class="crow fav-crow" data-id="'+f.id+'"><div class="crow-poster">'+(poster?'<img src="'+poster+'" alt="" loading="lazy">':'<div class="crow-fb">'+f.title+'</div>')+'</div><div class="crow-info"><div class="crow-title">'+f.title+'</div><div class="crow-meta">'+tl+(sn?' · '+sn:'')+epLabel+'</div></div><button class="mylist-remove-btn" data-id="'+f.id+'" aria-label="Verwijder">✕</button></div>';}
