...
function fetchSpinDiscover(){
  ...
  Object.keys(streamerPids).forEach(function(pk){
    ...
    /* Fetch movies and tv for this streamer */
    fetches.push(tmdb('/discover/movie',{with_watch_providers:pids,watch_region:'NL',language:'nl-NL',sort_by:'popularity.desc',page:1,'primary_release_date.gte':'1980-01-01'}).catch(function(){return{results:[]};}));
    fetchKeys.push({pk:pk,type:'movie'});
    fetches.push(tmdb('/discover/tv',{with_watch_providers:pids,watch_region:'NL',language:'nl-NL',sort_by:'popularity.desc',page:1,'first_air_date.gte':'1980-01-01'}).catch(function(){return{results:[]};}));
...
function doSpin(sec){
  ...
  Promise.all([fetchSpinPool(),fetchSpinDiscover()]).then(function(res){
    ...
    /* If streamer filter active, prioritize discover results for those streamers */
    ...
    /* If no streamer filter, also mix in ALL discover items for variety */
    if(!spinSelectedStreamers.length){
    ...
    /* Deprioritize already-seen-this-session items (push to back, don't remove) */
    var fresh=[];var stale=[];
    candidates.forEach(function(c){
      if(spinSeenIds[c.id])stale.push(c);
      else fresh.push(c);
    });
    /* If we still have fresh items, use those; otherwise fall back to stale */
    ...
    /* Pick and verify NL streaming — allow up to 30 attempts for larger pool */
    var attempts=0;var maxAttempts=Math.min(pool.length,30);
