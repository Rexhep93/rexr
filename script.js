...
function drawHeaderLogo(){
  ...
  wrapper.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="-40 -1051 4672 1444" style="height:2.6em;width:auto;display:block;">
...
function addToMyList(item){
  ...
  myList.push({id:item.id,title:item.title,img:item.img||imgCache[item.id]||'',_type:item._type,_key:item._key,_src:item._src,tmdb_id:item.tmdb_id||null,_genres:item._genres||[],_date:item._date||'',_source:item._source||'tmdb'});
...
function crowHtml(item){
  ...
  var epBadge='';
  if(item._type==='tv'){var ep=item._epInfo;var hasSeason=!!item._season;var hasEpInfo=!!(ep&&ep.s);var isGS=item._source==='googlesheet';var gsEp=item._gsEp||'';if(gsEp&&gsEp.match(/^S\d+E\d+$/)){var gsParts=gsEp.match(/^S(\d+)E(\d+)$/);var gsS=parseInt(gsParts[1]);var gsE=parseInt(gsParts[2]);var gsTot=ep&&ep.seasonEpisodeCount?ep.seasonEpisodeCount:null;epBadge='<div class="crow-ep-badge">Nieuwe aflevering '+gsE+(gsTot?' van '+gsTot:'')+'</div><div class="crow-ep-badge season">Seizoen '+gsS+'</div>';}else if(gsEp&&gsEp.match(/^S\d+$/)){var gsS2=parseInt(gsEp.replace('S',''));epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+gsS2+')</div>';}else if(hasEpInfo){var totalEps=ep.seasonEpisodeCount||null;var completeDrop=ep.isCompleteDrop||false;var fullSeasonAvailable=ep.airedEpisodes&&ep.seasonEpisodeCount&&ep.airedEpisodes>=ep.seasonEpisodeCount&&!ep.hasNextEpisode;if(ep.e===1||completeDrop||fullSeasonAvailable){epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';}else{if(totalEps){epBadge='<div class="crow-ep-badge">Nieuwe aflevering '+ep.e+' van '+totalEps+'</div>';}else{epBadge='<div class="crow-ep-badge">Nieuwe aflevering</div>';}epBadge+='<div class="crow-ep-badge season">Seizoen '+ep.s+'</div>';}}else if(hasSeason){epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+item._season+')</div>';}else if(isGS){epBadge='<div class="crow-ep-badge">Nieuwe aflevering</div>';}}
...
function openModal(rawId){
  ...
  chain.then(function(){
    ...
    var chips=[];var imdbUrl=imdbId?'https://www.imdb.com/title/'+imdbId+'/':'https://www.imdb.com/find/?q='+encodeURIComponent(fullTitle)+'&s=tt';chips.push('<a class="rating-chip imdb" href="'+imdbUrl+'" target="_blank" rel="noopener"><span class="chip-logo">IMDb</span>Bekijk op IMDb</a>');chips.push('<a class="rating-chip rt" href="https://www.rottentomatoes.com/search?search='+encodeURIComponent(fullTitle)+'" target="_blank" rel="noopener"><span class="chip-logo">RT</span>Bekijk op RT</a>');ratingsRow.innerHTML=chips.join('');ratingsRow.style.display='flex';
...
/* ── MY LIST ── */
function renderMyListItem(f,mode,extraClass){var poster=f.img||'';var tl=f._type==='movie'?'Film':'Serie';var sn=capitalizeProvider(f._src&&f._src.name||'');var epLabel='';if(f._type==='tv'){if(mode==='next'&&f._nextEp){epLabel=' · '+f._nextEp;}else if(f._fullSeason){epLabel=' · S'+String(f._fullSeason).padStart(2,'0')+' - volledig seizoen';}else if(f._lastEp){epLabel=' · t/m '+f._lastEp;}}return '<div class="crow fav-crow'+(extraClass?' '+extraClass:'')+'" data-id="'+f.id+'"><div class="crow-poster">'+(poster?'<img src="'+poster+'" alt="" loading="lazy">':'<div class="crow-fb">'+f.title+'</div>')+'</div><div class="crow-info"><div class="crow-title">'+f.title+'</div><div class="crow-meta">'+tl+(sn?' · '+sn:'')+epLabel+'</div></div><button class="mylist-remove-btn" data-id="'+f.id+'" aria-label="Verwijder">✕</button></div>';}
function enrichMyListDates(callback){
  var today=todayISO();
  function epStr(ep){return ep?'S'+String(ep.s).padStart(2,'0')+'E'+String(ep.e).padStart(2,'0'):null;}
  function isCompleteDrop(ep){return ep&&ep.airedEpisodes&&ep.seasonEpisodeCount&&ep.airedEpisodes>=ep.seasonEpisodeCount;}
  myList.filter(function(f){return f._type==='tv'&&f.tmdb_id;}).forEach(function(item){
    var isGS=item._source==='googlesheet';
    if(isGS){
      var gsAll=allItems.filter(function(i){return i._source==='googlesheet'&&i._type==='tv'&&String(i.tmdb_id)===String(item.tmdb_id);}).sort(function(a,b){return a._date.localeCompare(b._date);});
      var current=null;var nextItem=null;
      gsAll.forEach(function(i){if(i._date<=today)current=i;else if(!nextItem)nextItem=i;});
      item._date=current?current._date:today;
      var cEp=current?current._epInfo:null;
      if(!current._gsEp&&isCompleteDrop(cEp)){item._fullSeason=cEp.s;item._lastEp=null;}
      else{item._fullSeason=null;item._lastEp=current?(current._gsEp||epStr(cEp)):null;}
      item._nextDate=nextItem?nextItem._date:null;
      item._nextEp=nextItem?(nextItem._gsEp||epStr(nextItem._epInfo)):null;
    } else {
      var liveItem=allItems.find(function(i){return String(i.tmdb_id)===String(item.tmdb_id)&&i._type==='tv'&&i._source!=='googlesheet';});
      if(liveItem&&liveItem._date){
        if(liveItem._date>today){
          item._nextDate=liveItem._date;item._nextEp=epStr(liveItem._epInfo);
          item._date=today;item._lastEp=null;item._fullSeason=null;
        } else {
          item._date=liveItem._date;
          var lEp=liveItem._epInfo;
          if(isCompleteDrop(lEp)){item._fullSeason=lEp.s;item._lastEp=null;}
          else{item._fullSeason=null;item._lastEp=epStr(lEp);}
          item._nextDate=null;item._nextEp=null;
        }
      }
    }
  });
  myList.filter(function(f){return f._type==='tv'&&!f.tmdb_id;}).forEach(function(item){
    var gsAll=allItems.filter(function(i){return i._source==='googlesheet'&&i._type==='tv'&&i.title===item.title;}).sort(function(a,b){return a._date.localeCompare(b._date);});
    var current=null;var nextItem=null;
    gsAll.forEach(function(i){if(i._date<=today)current=i;else if(!nextItem)nextItem=i;});
    if(current){item._date=current._date;item._lastEp=current._gsEp||null;item._fullSeason=null;}
    if(nextItem){item._nextDate=nextItem._date;item._nextEp=nextItem._gsEp||null;}
  });
  callback();
}
...
function renderSpinKijk(){
  ...
  sec.innerHTML='<div class="spin-hdr"><div class="spin-title"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="1.8"><circle cx="12" cy="13" r="9"/><line x1="12" y1="4" x2="12" y2="13"/><line x1="3" y1="13" x2="12" y2="13"/><line x1="21" y1="13" x2="12" y2="13"/><line x1="5.36" y1="6.36" x2="12" y2="13"/><line x1="18.64" y1="6.36" x2="12" y2="13"/><line x1="5.36" y1="19.64" x2="12" y2="13"/><line x1="18.64" y1="19.64" x2="12" y2="13"/><line x1="12" y1="22" x2="12" y2="13"/><polygon points="12,1 10,4 14,4" fill="var(--green)" stroke="none"/></svg> Spin & kijk</div><div class="spin-sub">Geen idee wat je vanavond kijkt? <br />Draai het wiel en laat het lot beslissen.</div></div>'+
    ...
    '<div class="spin-stage" id="spinStage">'+
      '<div class="spin-idle">'+
        '<div class="spin-wheel-wrap"><div class="spin-wheel" id="spinWheel">'+
         '<svg width="160" height="160" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><defs><filter id="f" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dy=".5" stdDeviation=".4" flood-color="#0a2e12" flood-opacity=".35"/></filter><clipPath id="c"><circle cx="12" cy="12" r="8.5"/></clipPath></defs><circle cx="12" cy="12" r="11.5" fill="#0b401a"/><circle cx="12" cy="12" r="10.8" fill="#145c28"/><g clip-path="url(#c)"><g transform="translate(12 12)"><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#e63946"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#f4a261" transform="rotate(22.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#e9c46a" transform="rotate(45)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#2a9d8f" transform="rotate(67.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#48cae4" transform="rotate(90)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#4361ee" transform="rotate(112.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#7b2d8b" transform="rotate(135)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#e040fb" transform="rotate(157.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#ff6b6b" transform="rotate(180)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#ffd166" transform="rotate(202.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#06d6a0" transform="rotate(225)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#118ab2" transform="rotate(247.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#3a86ff" transform="rotate(270)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#8338ec" transform="rotate(292.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#fb5607" transform="rotate(315)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#38b000" transform="rotate(337.5)"/></g></g><circle cx="12" cy="12" r="8.5" fill="none" stroke="#0f5422" stroke-width=".3"/><g transform="translate(12 12)"><circle cx="9.7" cy="0" r=".55" fill="#ffe14d"/><circle cx="6.86" cy="6.86" r=".55" fill="#ffc107"/><circle cx="0" cy="9.7" r=".55" fill="#ffe14d"/><circle cx="-6.86" cy="6.86" r=".55" fill="#ffc107"/><circle cx="-9.7" cy="0" r=".55" fill="#ffe14d"/><circle cx="-6.86" cy="-6.86" r=".55" fill="#ffc107"/><circle cx="0" cy="-9.7" r=".55" fill="#ffe14d"/><circle cx="6.86" cy="-6.86" r=".55" fill="#ffc107"/><circle cx="8.95" cy="3.71" r=".45" fill="#ffe14d"/><circle cx="3.71" cy="8.95" r=".45" fill="#ffc107"/><circle cx="-3.71" cy="8.95" r=".45" fill="#ffe14d"/><circle cx="-8.95" cy="3.71" r=".45" fill="#ffc107"/><circle cx="-8.95" cy="-3.71" r=".45" fill="#ffe14d"/><circle cx="-3.71" cy="-8.95" r=".45" fill="#ffc107"/><circle cx="3.71" cy="-8.95" r=".45" fill="#ffe14d"/><circle cx="8.95" cy="-3.71" r=".45" fill="#ffc107"/></g><g filter="url(#f)"><polygon points="12,2.6 10.3,0 13.7,0" fill="#0b401a" stroke="#2da547" stroke-width=".35" stroke-linejoin="round"/></g><circle cx="12" cy="12" r="2.2" fill="#0f5422" stroke="#1a7a32" stroke-width=".4"/><circle cx="12" cy="12" r="1.4" fill="#0b401a" stroke="#2da547" stroke-width=".5"/><circle cx="12" cy="12" r=".7" fill="#2da547"/></svg>'+
        '</div></div>'+
...
function doSpin(sec){
  ...
  stage.innerHTML='<div class="spin-spinning"><div class="spin-wheel-wrap"><div class="spin-wheel spinning" id="spinWheel">'+
    '<svg width="160" height="160" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><defs><filter id="f" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dy=".5" stdDeviation=".4" flood-color="#0a2e12" flood-opacity=".35"/></filter><clipPath id="c"><circle cx="12" cy="12" r="8.5"/></clipPath></defs><circle cx="12" cy="12" r="11.5" fill="#0b401a"/><circle cx="12" cy="12" r="10.8" fill="#145c28"/><g clip-path="url(#c)"><g transform="translate(12 12)"><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#e63946"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#f4a261" transform="rotate(22.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#e9c46a" transform="rotate(45)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#2a9d8f" transform="rotate(67.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#48cae4" transform="rotate(90)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#4361ee" transform="rotate(112.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#7b2d8b" transform="rotate(135)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#e040fb" transform="rotate(157.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#ff6b6b" transform="rotate(180)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#ffd166" transform="rotate(202.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#06d6a0" transform="rotate(225)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#118ab2" transform="rotate(247.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#3a86ff" transform="rotate(270)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#8338ec" transform="rotate(292.5)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#fb5607" transform="rotate(315)"/><path d="M0 0L0-9A9 9 0 0 1 2.77-8.56Z" fill="#38b000" transform="rotate(337.5)"/></g></g><circle cx="12" cy="12" r="8.5" fill="none" stroke="#0f5422" stroke-width=".3"/><g transform="translate(12 12)"><circle cx="9.7" cy="0" r=".55" fill="#ffe14d"/><circle cx="6.86" cy="6.86" r=".55" fill="#ffc107"/><circle cx="0" cy="9.7" r=".55" fill="#ffe14d"/><circle cx="-6.86" cy="6.86" r=".55" fill="#ffc107"/><circle cx="-9.7" cy="0" r=".55" fill="#ffe14d"/><circle cx="-6.86" cy="-6.86" r=".55" fill="#ffc107"/><circle cx="0" cy="-9.7" r=".55" fill="#ffe14d"/><circle cx="6.86" cy="-6.86" r=".55" fill="#ffc107"/><circle cx="8.95" cy="3.71" r=".45" fill="#ffe14d"/><circle cx="3.71" cy="8.95" r=".45" fill="#ffc107"/><circle cx="-3.71" cy="8.95" r=".45" fill="#ffe14d"/><circle cx="-8.95" cy="3.71" r=".45" fill="#ffc107"/><circle cx="-8.95" cy="-3.71" r=".45" fill="#ffe14d"/><circle cx="-3.71" cy="-8.95" r=".45" fill="#ffc107"/><circle cx="3.71" cy="-8.95" r=".45" fill="#ffe14d"/><circle cx="8.95" cy="-3.71" r=".45" fill="#ffc107"/></g><circle cx="12" cy="12" r="2.2" fill="#0f5422" stroke="#1a7a32" stroke-width=".4"/><circle cx="12" cy="12" r="1.4" fill="#0b401a" stroke="#2da547" stroke-width=".5"/><circle cx="12" cy="12" r=".7" fill="#2da547"/></svg>'+
